...
const admin=createClient(SUPABASE_URL,adminKey,{auth:{autoRefreshToken:false,persistSession:false,detectSessionInUrl:false}})
const allRoles=["superadmin","admin_ente","rup","tecnico","amministrativo","direttore_lavori","auditor","consultatore","manutentore","siservizi","dirigente_scolastico","delegato_scolastico"]
const managedRoles=allRoles.filter(r=>r!=="superadmin")
function cleanUser(user:any,profile:any,roles:any[]){return{id:user.id,email:user.email||profile?.email||"",nome:profile?.nome||user.user_metadata?.nome||"",cognome:profile?.cognome||user.user_metadata?.cognome||"",telefono:profile?.telefono||"",attivo:profile?.attivo!==false&&!user.banned_until,email_confirmed_at:user.email_confirmed_at,last_sign_in_at:user.last_sign_in_at,created_at:user.created_at,roles:roles.map(r=>({ente_id:r.ente_id,ente:r.enti?.denominazione||"",ruolo:r.ruolo}))}}
async function currentActor(req:Request){const auth=req.headers.get("Authorization")||"",token=auth.startsWith("Bearer ")?auth.slice(7):"";if(!token)throw new Error("Autenticazione richiesta");const {data,error}=await admin.auth.getUser(token);if(error||!data.user)throw new Error("Sessione non valida");const actorId=data.user.id;const {data:actorRoles,error:roleError}=await admin.from("user_roles").select("ente_id,ruolo").eq("user_id",actorId);if(roleError)throw new Error(roleError.message);const isSuperadmin=actorRoles?.some(r=>r.ruolo==="superadmin")||false;const managedEnteIds=(actorRoles||[]).filter(r=>r.ruolo==="superadmin"||r.ruolo==="admin_ente").map(r=>r.ente_id);if(!isSuperadmin&&!managedEnteIds.length)throw new Error("Utente non autorizzato alla gestione degli utenti");return{actorId,isSuperadmin,managedEnteIds}}
async function canManageTarget(actor:any,targetUserId:string,enteId?:string){if(actor.isSuperadmin)return true;if(!enteId||!actor.managedEnteIds.includes(enteId))return false;const {data,error}=await admin.from("user_roles").select("user_id,ente_id,ruolo").eq("user_id",targetUserId).eq("ente_id",enteId);if(error)throw new Error(error.message);return(data||[]).length>0&&!(data||[]).some(r=>r.ruolo==="superadmin")}
