...
type Page='dashboard'|'edifici'|'richieste'|'interventi'|'intervento'|'fascicolo'|'report'|'amministrazione'|'manutenzioni'|'manutenzioni-scadenze'
...
function MaintenancePage({access,refresh,setRefresh,initialBuildingId}:{access:Access[];refresh:number;setRefresh:(x:number)=>void;initialBuildingId?:string|null}){
   ...
   <div className="maintenance-workspace">
    ...
    <div className="maintenance-systems">
     {systemLabels.map(def=>{const s=getSystem(def.key);return <React.Fragment key={def.key}>{def.key==='elevatore'&&<div className="maintenance-section-heading"><div><span className="eyebrow">Sezione tecnica</span><h2>Impianti elevatori</h2><p>Ascensori, piattaforme elevatrici e relativi interventi di manutenzione e verifica.</p></div><Wrench size={20}/></div>}<section id={'maintenance-'+def.key} className={'card maintenance-system-card '+(def.key==='elevatore'?'maintenance-elevator-card':'')}>
      ...
      <div className="maintenance-card-actions">{s?(canUpdate&&<button className="btn secondary" onClick={()=>{setEditingSystem(s);setShowSystem(true)}}><Pencil size={15}/> Modifica</button>):(canCreate&&<button className="btn secondary" onClick={()=>{setEditingSystem(null);setShowSystem(true)}}><Plus size={15}/> Censisci impianto</button>)}{s&&canDelete&&<button className="icon-btn" title="Elimina scheda" onClick={()=>removeSystem(s)}><Trash2 size={16}/></button>}</div>
...
type MaintenanceAlertRow={id:string;source:'impianto'|'manutenzione';edificio_id:string;edificio:string;codice_edificio:string;tipo:string;descrizione:string;data:string;stato:string;ditta:string|null;periodicita:number|null;days:number}
...
function MaintenanceAlertsPage({access,onOpen,refresh}:{access:Access[];onOpen:(p:Page,id?:string)=>void;refresh:number}){
 ...
 const [rows,setRows]=useState<MaintenanceAlertRow[]>([]);
 ...
 const enteId=access[0]?.ente_id;
 useEffect(()=>{let cancelled=false;const load=async()=>{setLoading(true);const [b,s,e]=await Promise.all([supabase.from('edifici').select('id,codice_edificio,denominazione').order('denominazione'),supabase.from('impianti_manutentivi').select('id,edificio_id,tipo,denominazione,stato,data_prossima_manutenzione,ditta_manutentrice,periodicita_mesi'),supabase.from('manutenzioni').select('id,edificio_id,impianto_id,tipo,stato,data_programmata,descrizione,operatore')]);if(cancelled)return;const buildings=(b.data||[]) as any[];const buildingMap=new Map(buildings.map(x=>[x.id,x]));const today=new Date();today.setHours(0,0,0,0);const diff=(value:string)=>{const d=new Date(value+'T00:00:00');return Math.round((d.getTime()-today.getTime())/86400000)};const next:MaintenanceAlertRow[]=[];(s.data||[]).forEach((x:any)=>{if(!x.data_prossima_manutenzione)return;const bd=buildingMap.get(x.edificio_id);if(!bd)return;const days=diff(x.data_prossima_manutenzione);if(days<=alertDays)next.push({id:x.id,source:'impianto',edificio_id:x.edificio_id,edificio:bd.denominazione,codice_edificio:bd.codice_edificio||'',tipo:x.tipo||'altro',descrizione:x.denominazione||'Manutenzione impianto',data:x.data_prossima_manutenzione,stato:x.stato||'attivo',ditta:x.ditta_manutentrice||null,periodicita:x.periodicita_mesi||null,days})});(e.data||[]).forEach((x:any)=>{if(!x.data_programmata||['chiusa','annullata'].includes(x.stato))return;const bd=buildingMap.get(x.edificio_id);if(!bd)return;const days=diff(x.data_programmata);if(days<=alertDays)next.push({id:x.id,source:'manutenzione',edificio_id:x.edificio_id,edificio:bd.denominazione,codice_edificio:bd.codice_edificio||'',tipo:x.tipo||'altro',descrizione:x.descrizione||'Manutenzione programmata',data:x.data_programmata,stato:x.stato||'programmata',ditta:x.operatore||null,periodicita:null,days})});next.sort((a,b)=>a.days-b.days||a.edificio.localeCompare(b.edificio));setRows(next);setSelected(prev=>prev&&next.some(x=>x.id===prev.id&&x.source===prev.source)?prev:(next[0]||null));setLoading(false)};void load();return()=>{cancelled=true}},[refresh,enteId,alertDays]);
