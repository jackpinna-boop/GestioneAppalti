...
type Page='dashboard'|'edifici'|'richieste'|'interventi'|'fascicolo'|'report'|'amministrazione'|'manutenzioni'
...
type RequestRow={
 ...
 note_immobile:string|null; note_risoluzione:string|null; data_richiesta:string;
...
function RequestsPage({access,refresh,setRefresh}:{access:Access[];refresh:number;setRefresh:(x:number)=>void}){
 ...
 const save=async(e:any)=>{
  ...
  const payload={
   ...
   tipo_intervento:String(f.get('tipo')||'da_valutare'),risolto,data_risoluzione:risolto?(String(f.get('data_risoluzione')||'')||new Date().toISOString().slice(0,10)):null,
   ...
   note_risoluzione:String(f.get('note_risoluzione')||'').trim()||null
  };
  setLoading(true);
 ...
 return <PageHead title="Richieste di intervento" subtitle="Segnalazioni provenienti dagli istituti scolastici e loro gestione.">
  ...
  <section className="card table-card">
   ...
   {filtered.map(x=>{const sedi=x.richieste_intervento_sedi||x.richieste_intervento_sedi||[];const amb=x.richieste_intervento_ambiti||[];const docs=x.richiesta_intervento_documenti||[];return <tr key={x.id} className="clickable" onClick={()=>setDetail(x)}>
    ...
    <td><span className={'badge '+requestTypeClass(x.tipo_intervento)}>{requestTypeLabel(x.tipo_intervento)}</span></td><td>{date(x.data_richiesta)}</td>
...
function RequestsDashboard({access,refresh,onOpen}:{access:Access[];refresh:number;onOpen:(p:Page,id?:string)=>void}){
 const [rows,setRows]=useState<RequestRow[]>([]);const [buildings,setBuildings]=useState<any[]>([]);const [period,setPeriod]=useState<'giorno'|'settimana'|'mese'|'anno'>('mese');const [building,setBuilding]=useState('all');const [hover,setHover]=useState<number|null>(null);
 useEffect(()=>{Promise.all([
  supabase.from('richieste_intervento').select('id,data_richiesta,risolto,richieste_intervento_sedi(edificio_id)'),
  supabase.from('edifici').select('id,codice_edificio,denominazione').order('denominazione')
 ]).then(([a,b])=>{setRows((a.data||[]) as RequestRow[]);setBuildings(b.data||[])})},[refresh]);
 ...
 const now=new Date(); const points=useMemo(()=>{
  ...
  const push=(label:string,d:Date)=>out.push({label,count:scoped.filter(r=>{const x=new Date(r.data_richiesta+'T00:00:00');return period==='giorno'?x.toDateString()===d.toDateString():period==='settimana'?weekKey(x)===weekKey(d):period==='mese'?(x.getFullYear()===d.getFullYear()&&x.getMonth()===d.getMonth()):(x.getFullYear()===d.getFullYear())}).length,date:d});
  const n=period==='giorno'?30:period==='settimana'?12:period==='mese'?12:6;
  for(let i=n-1;i>=0;i--){const d=new Date(now);if(period==='giorno')d.setDate(now.getDate()-i);if(period==='settimana')d.setDate(now.getDate()-i*7);if(period==='mese')d.setMonth(now.getMonth()-i,1);if(period==='anno')d.setFullYear(now.getFullYear()-i,0,1);push(period==='giorno'?fmtDay(d):period==='settimana'?fmtWeek(d):period==='mese'?fmtMonth(d):String(d.getFullYear()),d)}
  return out;
 },[rows,building,period]);
 const max=Math.max(1,...points.map(p=>p.count)); const W=760,H=260,pad=38;const pts=points.map((p,i)=>({x:pad+i*(W-pad*2)/Math.max(1,points.length-1),y:H-pad-(p.count/max)*(H-pad*2),...p}));
...
function AdminPage({access}:{access:Access[]}){
 ...
 return <PageHead title="Amministrazione" subtitle="Utenti, enti, ruoli e configurazione di accesso.">
   ...
   <section id="admin-audit" className="card">
