...
export default function SchoolPortalPage({session,access}:{session:any;access:Access[]}){
 ...
 const requestCodeById=useMemo(()=>Object.fromEntries(rows.map(r=>[r.id,r.codice_richiesta||'—'])),[rows])

 const create=async(e:React.FormEvent<HTMLFormElement>)=>{
   e.preventDefault();setSaving(true);setMessage('')
   const f=new FormData(e.currentTarget)
   const buildingIds=[...new Set(f.getAll('edificio').map(x=>String(x)).filter(Boolean))]
   if(!buildingIds.length){setMessage('Seleziona almeno un edificio.');setSaving(false);return}
   ...
   const {data,error}=await supabase.rpc('school_create_request_multi',{
     p_ente_id:access[0]?.ente_id,
     p_edificio_ids:buildingIds,
