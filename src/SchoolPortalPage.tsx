 ...
 return <div className="school-portal">
   ...
   <div className="school-portal-grid">
    <section className="card">
     ...
     {loading?<div className="loading-screen">Caricamento…</div>:filtered.length?<div className="table-card"><table><thead><tr><th>Codice</th><th>Richiesta</th><th>Edificio</th><th>Data</th><th>Stato</th><th></th></tr></thead><tbody>{filtered.map(r=>{const sedi=r.richieste_intervento_sedi||[];return <tr key={r.id} className="clickable"><td>{r.codice_richiesta||'—'}</td><td><b>{r.titolo_sintetico}</b><span className="table-sub">{r.priorita||'ordinaria'}</span></td><td>{sedi.length?sedi.map((s:any)=>s.edifici?.denominazione||'—').join(', '):'—'}</td><td>{fmtDate(r.data_richiesta)}</td><td><span className={'badge '+(r.stato_risoluzione==='risolta'?'green':r.stato_risoluzione==='annullata'?'gray':'amber')}>{r.stato_risoluzione}</span></td><td><button className="btn secondary small" onClick={()=>void openRequest(r)}>Apri</button></td></tr>})}</tbody></table></div>:<div className="empty"><FileText size={28}/><strong>Nessuna richiesta</strong><span>Utilizza “Segnala intervento” per inserire una nuova segnalazione.</span></div>}
    </section>
