...
const isoDate=(v:any)=>{const s=clean(v);if(!s)return null;const m=s.match(/^(\d{1,2})[-/](\d{1,2})[-/](\d{2,4})$/);if(m){let y=Number(m[3]);if(y<100)y+=2000;return y+'-'+String(m[2]).padStart(2,'0')+'-'+String(m[1]).padStart(2,'0')}const p=s.toLowerCase().replace(/\./g,'').split('-');if(p.length===3){const months:any={gen:1,feb:2,mar:3,apr:4,mag:5,giu:6,lug:7,ago:8,set:9,ott:10,nov:11,dic:12};const dd=Number(p[0]),mm=months[p[1].slice(0,3)],yy=Number(p[2]);if(dd&&mm&&yy){const yyyy=yy<100?2000+yy:yy;return yyyy+'-'+String(mm).padStart(2,'0')+'-'+String(dd).padStart(2,'0')}}const d=new Date(s);return Number.isNaN(d.getTime())?null:d.toISOString().slice(0,10)}
